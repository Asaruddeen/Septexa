...
router.get('/search', protect, (req, res) => {
  ...
  if (q && q.trim() !== '') {
    ...
    results = results.filter(m =>
      ...
      m.cost.toLowerCase().includes(query)
    );
  }

  if (filter && filter !== 'all') {
    results = results.filter(m => m.costLevel === filter);
  }
...
router.get('/category/:category', protect, (req, res) => {
  ...
  const results = toolsData.filter(m => m.category === category);
  res.json(results);
});
...
router.get('/:name', protect, (req, res) => {
  ...
  const tool = toolsData.find(m => m.name.toLowerCase() === name.toLowerCase());
  if (!tool) {
